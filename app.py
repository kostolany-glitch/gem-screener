...
with tab3:
    ...
    if len(dual) == 0:
        ...
        st.markdown("---")
        dual_summary = build_claude_summary(dual, COL, label="雙達標")
        st.text_area(
            "📋 複製以下內容貼給 Claude 進行進場分析",
            value=dual_summary,
            height=300,
            key="dual_copy"
        )
        st.caption("全選文字（Ctrl+A 或三擊）→ 複製 → 貼給 Claude，附上你對這些標的的提問即可。")
    ...
    if len(near) == 0:
        ...
        st.markdown("---")
        near_summary = build_claude_summary(near, COL, label="接近門檻")
        st.text_area(
            "📋 複製以下內容貼給 Claude 進行追蹤分析",
            value=near_summary,
            height=300,
            key="near_copy"
        )
        st.caption("全選文字（Ctrl+A 或三擊）→ 複製 → 貼給 Claude，詢問需等待哪些條件成熟。")
